...
| [Extending Claude Code](extensions.md) | Seven extension mechanisms (CLAUDE.md, Skills, MCP, Subagents, Agent Teams, Hooks, Plugins), when to use each, how they layer, and context-cost trade-offs | 2026-04-22 |
...
| [Custom Subagents](subagents.md) | Full subagent configuration reference — frontmatter fields, storage scopes and priority, foreground vs. background, lifecycle hooks, built-ins | 2026-04-22 |
| [Agent Teams](agent-teams.md) | Full agent teams reference — architecture (lead/teammates/task list/mailbox), display modes, task assignment, plan approval, quality-gate hooks (TeammateIdle/TaskCreated/TaskCompleted), subagent-definition reuse as teammates, best practices, and limitations | 2026-04-22 |
...
| [Skills and Slash Commands](slash-commands.md) | Full skills reference — frontmatter fields, invocation control (disable-model-invocation, user-invocable), argument substitution ($ARGUMENTS/$N), bash injection, context:fork subagent execution, scope/priority, and sharing | 2026-04-19 |
| [tmux-cli](tmux-cli.md) | Third-party CLI tool (claude-code-tools) for agent control of tmux panes — launch, send, capture, wait_idle, kill; local/remote mode; shell-first usage pattern | 2026-04-22 |
